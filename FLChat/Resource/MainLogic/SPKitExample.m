...
#if __has_include("SPLoginViewController.h")
...
//#warning TODO: CHANGE TO YOUR AppKey
        /// 线上环境，更换成你自己的AppKey
        [[YWAPI sharedInstance] syncInitWithOwnAppKey:kAPPKey getError:&error];
    } else {
...
#if __has_include("SPLoginViewController.h")
            UIViewController *loginViewController = [[SPLoginViewController alloc] initWithNibName:@"SPLoginViewController" bundle:nil];
...
{
//     __weak typeof(self) weakSelf = self;
    [[self.ywIMKit.IMCore getLoginService] asyncLogoutWithCompletionBlock:nil];
}
...
#warning TODO: CHANGE TO YOUR ACTUAL GETTING Custom Conversation Profile METHOD
        if (aCompletionBlock) {
            if ([conversation.conversationId isEqualToString:SPTribeSystemConversationID]) {
                ...
                //Am6注释
//                aCompletionBlock(YES, conversation, @"hehe云旺iOS精华问题大汇总！", [UIImage imageNamed:@"input_plug_ico_card_nor"]);
            }
        }
    }];
}
...
//        NSLog(@"conversationViewController:%@", conversationViewController);
        ...
        [mutArr addObjectsFromArray:conversationNavigationController.viewControllers];
        [mutArr addObject:conversationViewController];
        viewControllers = mutArr;
    }
